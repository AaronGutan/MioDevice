...
void setup() {
  ...
  myOLED.print("000", CENTER, 20);                 // Обнуляем значение веса на экране
...
void loop() {
  ...
  if (buttonState == HIGH) {
    ...
    //if (round(scale.get_units(10)) != myScale) {      // Условие выполняется, если округленный до граммов вес отличается от последнего
      ...
      if (myScale > 50)                              // Если вес не ничтожно мал
        myOLED.printNumI(myScale, CENTER, 20, 3);      // то выводим его
      else                                            // иначе
        myOLED.print("000", CENTER, 20);             // выводим нули
